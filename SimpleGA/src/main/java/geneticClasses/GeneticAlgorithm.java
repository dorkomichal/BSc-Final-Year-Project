...
public class GeneticAlgorithm {
    ...
    private static Population evolution(Population currentPopulation, boolean multipoint, int numberOfCrossPoints, boolean tournament) {
        ...
        if (elitism) {
            newGeneration.saveIndividual(currentPopulation.getFittestIndividual(), 0);
            allocated = 1;
        }
        for (int i = allocated; i < currentPopulation.getSizeOfPopulation() -1; i += 2) {
            ...
            if (Math.random() <= crossoverRate) {
                ...
                offspring = new Individual[]{parent1, parent2};
            }
            newGeneration.saveIndividual(offspring[0], allocated);
            newGeneration.saveIndividual(offspring[1], allocated + 1);
            allocated += 2;
        ...
        for (int j = allocated; j < newGeneration.getSizeOfPopulation(); j++) {
            ...
            newGeneration.saveIndividual(currentPopulation.getIndividual(randomNumber), allocated);
            allocated ++;
        }
    ...
    private static Individual[] multiPointCrossover(Individual parent1, Individual parent2, int numberOfPoints) {
     ...
     */
    protected static void mutate(Individual individual) {
     ...
     */
    private static Individual tournamentSelection(Individual competitor1, Individual competitor2) {
        double r = Math.random();
        Individual fitter = fitterFromTwo(competitor1, competitor2);
     ...
     */
    private static Individual fitterFromTwo(Individual individual1, Individual individual2) {
     ...
     */
    public static Individual rwsSelection(Population population, boolean firstRound) {
        if (firstRound) {
